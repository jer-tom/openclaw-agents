...
If you want requests to automatically refresh expired access tokens, use:

```bash
bash {baseDir}/scripts/strava_get.sh "/api/v3/athlete"
bash {baseDir}/scripts/strava_get.sh "/api/v3/athlete/activities?per_page=10"
```

This wrapper:
- reads tokens from `STRAVA_TOKEN_FILE` (default: `/data/.openclaw/credentials/strava-tokens.json`) if present
- otherwise uses `STRAVA_ACCESS_TOKEN` / `STRAVA_REFRESH_TOKEN`
- refreshes the access token when it’s expired (or on retryable 401s), and **updates the token file**

You still must authorize once with the right scopes (e.g. `activity:read_all`). Refreshing does not add scopes.
